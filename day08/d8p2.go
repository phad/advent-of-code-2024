...
// outside reports whether p lies outside g's bounds.
func outside(g *aoc.Grid[bool], p aoc.Point) bool {
	return !g.In(p)
}
	...
	for i, locN1 := range as.locations {
		for j := i + 1; j < len(as.locations); j++ {
			...
			for {
				...
				if outside(as.antinodes, locAN1) && outside(as.antinodes, locAN2) {
					break
				}
			}
		}
